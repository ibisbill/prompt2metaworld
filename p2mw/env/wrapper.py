...
class ExtendedTimeStepWrapper(dm_env.Environment):
    ...
    def observation_spec(self):
        return specs.BoundedArray(self._env.observation_space.shape, np.uint8, name='observation', minimum=0,
                                  maximum=255)
...
class MetaWorldWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        return self._state_obs, reward, False, info
...
def make(name, frame_stack, action_repeat, seed, train=False, cam_name='corner2', img_size=84, episode_length=200,
         device_id=-1):
    env = mw_gym_make(name + '-v2', cam_height=img_size, cam_width=img_size, cam_name=cam_name, train_distrib=train,
